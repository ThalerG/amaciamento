...
vars = {'cRMS','vSupRMS'}; % Nomes das variáveis
for kint = 1:(length(vars)+1)
    ...
    if kint ~= (length(vars)+1) % Vazão não foi excluída
        if kint == 1 % Todas as variáveis
            varnames = append(strcat(vartemp(1:(length(vars)-1)),'_'),num);
        else % Uma variável excluída
            varnames = append(strcat(vartemp(1:(length(vars)-2)),'_'),num);
        end
