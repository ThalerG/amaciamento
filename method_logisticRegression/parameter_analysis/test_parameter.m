...
N = 9; % Janela (número de amostras) da regressão
M = 90; % Janela da média móvel
D = 15; % Distância entre amostras da regressão
...
figure;
p = plot(Res.FPR,Res.TPR); xlim([0 1]); ylim([0 1]);hold on;
...
b = Res(3,12,32);
plot(b.FPR,b.TPR); xlim([0 1]); ylim([0 1]);
row = dataTipTextRow('Thr',thr);
p.DataTipTemplate.DataTipRows(end+1) = row;
ylabel('True Positive Rate TP/(TP+FN)');
xlabel('False Positive Rate FP/(FP+TN)');