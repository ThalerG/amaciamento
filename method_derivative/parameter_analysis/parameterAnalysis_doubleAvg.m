...
%    tAmac{k1} = cell(1,1); % For every run-in test (A2)
    for k2 = 1:length(tAmac{k1})
        tAmac{k1}{k2} = zeros(length(W1),length(W2),length(S),length(R),length(F));
    end
end
                ...
                for s = 1:length(S)  % For every maximum tolerance
                    ...
                    for f = 1:length(F)
                        ...
                        for r = 1:length(R)
                            ...
                            if isempty(temp)
                                tAmac{k1}{k2}(w1,w2,s,r:end,f) = NaN;
                                break
                            else
                                tAmac{k1}{k2}(w1,w2,s,r,f) = temp;
                            end
                        end
                    end
                end
...
J = zeros(length(W1),length(W2),length(S),length(R),length(F));
...
nameVar = {'$w_1$','$w_2$','$\bar{d_f}$','$r$','$f$'};
nvar = 5;
param = {W1,W2,S,R,F};

figure;
sz = 700; r = 1; gap = 20; marg_h = [45 10]; marg_w = [57 50];
...
savefig([fsave,'doubleAvg_custo_parametros.fig']);
vecrast(gcf,[fsave,'doubleAvg_custo_parametros'],800,'bottom','pdf');